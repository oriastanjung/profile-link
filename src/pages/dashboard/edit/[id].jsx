import { getLinksById, updateLink } from "@/api/services";
...
export default function Edit({ data, id }) {
  ...
  const [link, setLink] = useState({
    ...
    }${data?.attributes?.icon?.data?.attributes?.url.slice(1)}`,
    iconFile: "true",
    title: data.attributes?.title,
    status: data.attributes?.status,
    url: data.attributes?.url,
    account: data.attributes?.account.data.id,
  });
  ...
  return (
    <>
      ...
      >
        <h1 className="text-2xl font-extrabold">Update Link</h1>
        <div className="mt-12">
          {link.iconFile ? (
...
export async function getServerSideProps({ params }) {
  const { id } = params;
  const responseData = await getLinksById(id);
  return {
    props: {
